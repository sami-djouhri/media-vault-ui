...
# Leer = kein Samba-Nachzug (host). Auf node1 zeigt das auf vault-share.
SAMBA_COMPOSE = Path(os.environ["VAULT_SAMBA_COMPOSE"]) if os.environ.get("VAULT_SAMBA_COMPOSE") else None
READY_FILES = [
...
def mountpunkt_freiraeumen():
    ...
    Entfernt werden ausschliesslich LEERE Ordner. Liegen echte Daten darin,
    wird abgebrochen: ein Mount darueber wuerde sie unsichtbar machen, und
    unverschluesselt sind sie ohnehin am falschen Ort.
    """
    if is_mounted():
        return
    for eintrag in sorted(MOUNT_DIR.iterdir()):
        if eintrag.is_dir() and not any(eintrag.iterdir()):
            eintrag.rmdir()
    rest = sorted(e.name for e in MOUNT_DIR.iterdir())
    if rest:
        raise RuntimeError(
            "Mountpunkt ist nicht leer: " + ", ".join(rest) + ". "
            "Dort liegen unverschluesselte Daten, die ein Mount verdecken wuerde. "
            "Bitte wegsichern, dann erneut versuchen."
        )
...
def samba_loslassen():
    ...
    exportiert auch den Obsidian-Vault und die Musik.
    """
    if not SAMBA_COMPOSE or not SAMBA_COMPOSE.is_file():
        return
    run(["docker", "compose", "-f", str(SAMBA_COMPOSE), "stop", "vault-samba"], timeout=90)


def samba_nachziehen():
    """vault-samba neu starten, damit die Einspiel-Freigabe den Vault sieht.

    FUSE-Mounts propagieren nicht in bereits laufende Container: entsteht der
    Mount nach dem Containerstart, zeigt die Freigabe dauerhaft ins Leere.
    Ohne konfigurierte Compose-Datei (z. B. auf host) passiert nichts.
    """
    if not SAMBA_COMPOSE or not SAMBA_COMPOSE.is_file():
        return
    run(
        ["docker", "compose", "-f", str(SAMBA_COMPOSE),
         "up", "-d", "--force-recreate", "vault-samba"],
        timeout=120,
    )
...
def unmount_vault():
    ...
    if is_mounted():
        haltend = " ".join(mount_haltende_container())
        raise RuntimeError(
...
def mount_haltende_container():
    """Laufende Container, die den Mountpunkt gebunden haben.

    Ohne diese Angabe nennt die Fehlermeldung nur das Symptom und man sucht
    an der falschen Stelle.
    """
    namen = run(["docker", "ps", "--format", "{{.Names}}"], timeout=20)
    treffer = []
    for name in namen.stdout.split():
        binds = run(["docker", "inspect", name, "--format",
                     "{{range .Mounts}}{{.Source}} {{end}}"], timeout=20)
        if str(MOUNT_DIR) in binds.stdout:
            treffer.append(name)
    return treffer
